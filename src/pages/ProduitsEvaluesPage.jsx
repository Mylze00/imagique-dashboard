...
const ProduitsEvaluesPage = () => {
            ...
            data.produits.forEach((p) => {
              if (!allProducts.find((prod) => prod.code === p.code)) {
                allProducts.push({
                  ...p,
                  source: "commande",
                  prixCommande: p.total || p.prix || p.montant || 0,
                });
              }
            });
          }
  ...
  const fetchMoreProducts = async () => {
    ...
    const newData = [...produits, ...list];
    setProduits(newData);
    setLastDoc(snapshot.docs[snapshot.docs.length - 1] || null);

    localStorage.setItem(
      CACHE_KEY,
      JSON.stringify({ data: newData, lastDoc: snapshot.docs[snapshot.docs.length - 1] })
    );
        ...
        <main className="p-2 sm:p-4">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2">
            ...
            <div className="flex flex-wrap justify-center sm:justify-end gap-2 w-full sm:w-2/3">
              <select
                className="border border-gray-300 rounded-full px-4 py-2 text-xs"
          ...
          <div className="flex overflow-x-auto space-x-2 mb-4 py-2 no-scrollbar">
            {categoriesList.map((cat) => (
              <button
                ...
                className={`px-4 py-1 rounded-full text-xs whitespace-nowrap transition-all duration-200 ${
                  category === cat.key
          ...
          {isFetchingMore && (
            <p className="text-center mt-4 text-sm text-gray-500">Chargement plus de produits...</p>
          )}
        </main>
      </div>
    </div>
